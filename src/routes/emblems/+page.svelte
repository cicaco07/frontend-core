...
<div class="mx-auto max-w-7xl space-y-6 px-4 py-10 sm:px-6">
	...
	{#if data.emblems.length === 0}
		<p class="rounded-2xl border border-line bg-surface p-6 text-ink-muted">No emblems loaded yet. Connect the backend API.</p>
	{:else}
		<ul class="grid grid-cols-1 gap-4 md:grid-cols-2">
			{#each data.emblems as emblem (emblem.id)}
				<li class="rounded-2xl border border-line bg-surface/82 p-5 transition hover:-translate-y-1 hover:border-line-strong hover:bg-surface-2">
					<h2 class="font-display text-xl font-bold text-ink">{emblem.name}</h2>

					{#if statEntries(emblem.baseStats).length}
						<div class="mt-2">
							<p class="font-display text-xs font-bold tracking-wide text-ink-faint uppercase">Base Stats</p>
					...
					{#if emblem.talents.length}
						<div class="mt-3 border-t border-line pt-3">
							<p class="font-display text-xs font-bold tracking-wide text-ink-faint uppercase">Talents</p>
