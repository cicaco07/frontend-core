...
<div class="mx-auto max-w-7xl px-4 py-10 sm:px-6">
	<a href={resolve('/heroes')} class="text-sm text-ink-muted hover:text-accent">← Back to heroes</a>

	<div class="mt-4 grid gap-6 lg:grid-cols-[420px_1fr]">
		<div class="space-y-4">
			<div
				class="aspect-[4/3] overflow-hidden rounded-3xl border border-line bg-surface-3"
				style="border-left:4px solid {roleColor(hero.role)}"
			>
				{#if hero.imageUrl}
			...
			<div>
				...
				<p
					class="mt-1 flex items-center gap-2 text-sm capitalize"
					style="color:{roleColor(hero.role)}"
				>
					<span
